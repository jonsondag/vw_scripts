...
gdcmd="/usr/local/bin/vw -b 24 -q aa --total $mapred_map_tasks --node $mapper --unique_id $mapred_job_id --cache_file temp.cache --passes 1 --adaptive --exact_adaptive_norm -d /dev/stdin -f tempmodel --span_server $mapreduce_job_submithost --loss_function=logistic --l2 0.000001" 
mapred_job_id=`expr $mapred_job_id \* 2` #create new nonce
bfgscmd="/usr/local/bin/vw -b 24 -q aa --total $mapred_map_tasks --node $mapper --unique_id $mapred_job_id --cache_file temp.cache --bfgs --mem 5 --passes 20 --span_server $mapreduce_job_submithost -f model -i tempmodel --loss_function=logistic --l2 0.000001"
